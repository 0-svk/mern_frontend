...
const StockEnquiryForm = () => {
  ...
  const filteredProducts = Object.keys(stockData).filter((key) => {
    ...
    if (filter === "inStock" && stockData[key].status === "In Stock")
      return true;
    if (filter === "outOfStock" && stockData[key].status === "Out of Stock")
      return true;
    if (filter === "limitedStock" && stockData[key].status === "Limited Stock")
      return true;
    return false;
  });
        ...
        <div className="result">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((key) => (
              <p key={key}>
                {stockData[key].name} - {stockData[key].status}
              </p>
            ))
          ) : (
